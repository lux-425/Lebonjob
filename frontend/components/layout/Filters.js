...
const Filters = () => {
  ...
  function handleClick(checkbox) {
    if (typeof window !== 'undefined') {
      const checkboxType = document.getElementsByName(checkbox.name);

      checkboxType.forEach((itemType) => {
        if (itemType !== checkbox) itemType.checked = false;
      });
    }

    if (checkbox.checked === false) {
      // Delete the filter from query params
      ...
      // Set new filter's value if already there or not
      if (queryParams.has(checkbox.name)) {
