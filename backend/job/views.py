...
def updateJob(request, pk):
    ...
    if job.user != request.user:
        return Response({'message': 'You are not allowed to update this job.'}, status=status.HTTP_403_FORBIDDEN)
...
def deleteJob(request, pk):
    ...
    if job.user != request.user:
        return Response({'message': 'You are not allowed to delete this job.'}, status=status.HTTP_403_FORBIDDEN)
...
def applyToJob(request, pk):
    ...
    if user.userprofile.resume == '':
        return Response({"error": 'Please first upload your resume.'}, status=status.HTTP_400_BAD_REQUEST)

    if job.deadlineDate < timezone.now():
        return Response({"error": 'Sorry, the applying job deadline has been passed.'}, status=status.HTTP_400_BAD_REQUEST)
...
def isApplied(request, pk):
    ...
    isApplied = job.candidatesapplied_set.filter(user=user).exists()
...
def getCandidatesApplied(request, pk):
    ...
    if job.user != user:
        return Response({"error": "You can't access to the details of this job."}, status=status.HTTP_403_FORBIDDEN)
